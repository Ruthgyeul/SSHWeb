...
| `FilePreview` | Read-only preview modal for a remote file the server streams inline (never a client download until asked): image (data-URL `<img>`), video (data-URL `<video controls>`, e.g. mp4/mov/webm), or audio (data-URL `<audio controls>`, e.g. mp3/wav/flac); a file type the browser can't render opens in an `unsupported` mode that fetches nothing and just offers a download. `+download` in every mode |
