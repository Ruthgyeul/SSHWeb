...
import {
  useDownloadQueue,
  type DownloadJob,
} from "@/components/ssh/hooks/useDownloadQueue";

function job(path: string, resumeOffset = 0): DownloadJob {
  return { path, resumeOffset };
}

/** A queue whose starter always begins the job (consumes a slot). */
function setup(maxInFlight = 2) {
  const started: string[] = [];
  const start = vi.fn((j: DownloadJob) => {
    started.push(j.path);
    return true;
  });
  const hook = renderHook(() => useDownloadQueue(maxInFlight));
...
describe("useDownloadQueue", () => {
  ...
  it("passes the job's resumeOffset through to the starter", () => {
  ...
  it("skips a job the starter rejects without consuming a slot", () => {
    ...
    const hook = renderHook(() => useDownloadQueue(1));
    act(() => hook.result.current.setStart(start));
    act(() => {
